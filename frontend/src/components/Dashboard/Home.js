...
export default function Dashboard() {
        ...
        const fetchEvents = async () => {
          try {
            ...
            const processedSafetyData = data.safety_gear_data.map((safetyItem) => {
              ...
              const violationText = Object.keys(data)
                .filter((key) => key.startsWith('no-'))
                .map((key) => `${data[key]} No ${key.replace('no-', '')}`)
                .join(', ');

              const personText = `Person: ${data.person || 0}`;

              return {
                ...safetyItem,
                text: `${violationText}, ${personText}`,
              };
            });
  ...
  }, [loginType, setUserType, setUserId]);
